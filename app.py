...
def chart_layout(**overrides):
    """
    Safely merges CHART_STYLE with any per-chart overrides.

    Passing **CHART_STYLE and yaxis=... in the same update_layout() call
    causes a TypeError because 'yaxis' appears twice as a keyword argument.
    This helper removes any key from the base style that is also in overrides,
    then merges them — so callers can always do:

        fig.update_layout(**chart_layout(title="...", yaxis=dict(...)))
    """
    base = {k: v for k, v in CHART_STYLE.items() if k not in overrides}
...
# ══════════════════════════════════════════════════════════════════
    ...
    if "multi_result" in st.session_state:
        ...
        with c2:
            ...
            fig_cred.update_layout(**chart_layout(
